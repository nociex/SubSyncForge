...
### 修复
- **节点分类流程修复**：修复了IP检测后未回归到节点分类环节的问题，确保节点能够正确分类
- **订阅获取增强**：
  - 增加了订阅请求的超时处理和重试机制
  - 改进错误处理流程，支持自动重试请求
  - 使用AbortController优化请求超时处理
  - 增加请求重试等待间隔，提高成功率
- **容错机制增强**：
  - 修复单个订阅源失败导致整个同步过程中断的问题
  - 改进批量处理逻辑，使单个订阅源的失败不影响其他订阅源的处理
  - 添加失败订阅统计，提供更详细的错误报告
  - 增加最后成功节点缓存，用于错误恢复
...
### 修复
- **构建错误修复**：删除了NodeTester.js中对已移除的中国大陆IP相关模块的引用，解决了构建失败问题
- **IP定位功能增强**：优化了IPLocator.js中的API切换机制和错误处理，支持更多国内IP定位API

## [1.5.0] - 2025-05-23

### 重要变更
- **完全移除SOCKS代理验证功能**：因功能不稳定，完全移除相关代码
- **移除中国大陆IP检测与缓存**：删除中国大陆IP检测和缓存相关功能
- **增强订阅获取能力**：解决了特定订阅源无法获取的问题
- **优化节点测试逻辑**：提高测试的可靠性和准确性

### 新增
- **IP地理位置查询增强**：集成了多个免费IP地理位置API，提高定位准确性
- **代理节点状态缓存**：添加节点状态缓存，减少重复测试

### 优化
- **节点筛选策略优化**：改进了节点筛选算法，更精确识别节点特性
- **错误处理增强**：添加更详细的错误日志和失败重试机制
- **内存使用优化**：降低大量节点处理时的内存占用

### 修复
- **Base64解码问题**：修复了特定格式Base64编码内容解码失败的问题
- **HTTP请求超时处理**：改进了HTTP请求超时处理逻辑
- **节点名称乱码**：解决了部分节点名称出现乱码的问题

## [1.4.1] - 2025-05-22

### 重大改进
- **增强订阅获取能力**
  - 完全重写了`SubscriptionFetcher`类，增强了对各种订阅源的兼容性
  - 改进了请求头处理，解决了特定订阅源无法获取的问题
  - 增加了自动检测和处理Base64编码的能力
  - 增强了HTTP代理支持，支持自动切换代理服务器
  - 增加了智能重试和超时机制

### 优化
- **增强订阅解析能力**
  - 重写了`SubscriptionParser`类，增强了对多种格式的解析能力
  - 改进了`Base64Parser`，增加了对不规范Base64编码的处理
  - 优化了`PlainTextParser`，提高了解析速度和准确性
  - 增加了自动格式检测功能，可以智能识别订阅内容格式
  - 增强了错误处理和日志记录

### 新增
- 添加了测试脚本`test-subscription.js`，方便用户测试订阅获取和解析功能
- 增加了对更多协议的支持，包括Hysteria2和VLESS协议

### 文档
- 更新了README.md，提供了更详细的使用说明和配置指南
- 优化了代码注释，提高了代码可读性
...
- **代码结构完全重构**
  - 引入模块化架构，分离功能到独立模块
  - 创建核心功能模块：配置、订阅获取、节点处理、节点测试、配置生成、代理管理
  - 添加统一的同步管理器 (SyncManager) 协调各模块工作
  - 新增项目架构文档 (ARCHITECTURE.md)
...
### 优化
- 改进各订阅源的请求头处理，解决特定域名 (如 jikun.fun) 无法获取问题
- 增强节点解析能力，支持更多协议类型
- 重构配置生成逻辑，提高扩展性
- 优化错误处理流程，提供更详细的错误信息
- 提高代码可维护性和测试性
