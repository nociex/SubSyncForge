...
export class IPLocator {
  constructor(options = {}) {
    ...
    // 请求间隔控制 (毫秒)
    this.requestInterval = 1500; // 增加默认请求间隔到1.5秒
  ...
  async locate(ip) {
    ...
    for (const provider of this.apiProviders) {
      ...
      try {
        ...
        lastError = error;
        this.consecutiveFailures++; // 增加连续失败计数
  ...
  async rateLimitControl() {
    const now = Date.now();
    let interval = this.requestInterval;

    // 如果有连续失败，增加间隔时间 (指数退避)
    if (this.consecutiveFailures > 0) {
      const backoffConfig = Math.min(this.consecutiveFailures, 5); // 最多退避5次
      interval = this.requestInterval * Math.pow(1.5, backoffConfig);
      this.logger.debug(`由于连续失败 ${this.consecutiveFailures} 次，增加等待时间到 ${Math.round(interval)}ms`);
    }
    ...
    if (timeSinceLastRequest < interval) {
      ...
      // this.logger.debug(`频率控制等待 ${waitTime}ms`); // 减少日志噪音
